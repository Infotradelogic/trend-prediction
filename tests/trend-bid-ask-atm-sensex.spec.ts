...
test.skip('sensex::predict trend as per ATM bid and ask prices for CE and PE', 
 { tag: '@atm'},
  async () => {
  ...
  while (true) {  
    try {
      ...
      const POSITIVE_TOLERANCE_THRESHOLD = 15; 
      const NEGATIVE_TOLERANCE_THRESHOLD = -15; 
      
      // Create a new page
      const page = await browser.newPage();
      
      // Navigate to the Sensibull option chain URL for the specified expiry date
      await page.goto('https://web.sensibull.com/option-chain?tradingsymbol=SENSEX&expiry=2024-11-14');
      ...
      // Wait for additional data load and stabilization
      await page.waitForLoadState('networkidle');
      ...
      fs.appendFileSync('log_atm_sensex.txt', logData);
      ...
      // // Function to determine trend based on bid and offer prices for CE and PE
      // function getTrend(call_ce_bid, put_pe_bid, call_ce_offer, put_pe_offer) {
      //   if (call_ce_bid > put_pe_bid && call_ce_offer > put_pe_offer) {
      //     return "Up";
      //   } else if (call_ce_bid < put_pe_bid && call_ce_offer < put_pe_offer) {
      //     return "Down";
      //   } else {
